...
import { fetchGseTable } from './africanMarkets.js';
import { cachedStream } from './cache.js';
...
export async function streamAnalysis(analysisType, stockData, extraContext, onChunk) {
  ...
  // A custom extraContext makes this a one-off request — not worth caching/sharing.
  if (extraContext) {
    await run(onChunk);
    return;
  }

  await cachedStream(`analysis:${ticker}:${analysisType}`, ANALYSIS_TTL, run, onChunk);
}

export async function streamMarketDashboard(market, onChunk) {
  const run = async (chunkCb) => {
    const isGse = market === 'GSE';

    const [liveMap, filingMap] = await Promise.all([
      isGse ? fetchGseTable().catch(() => new Map()) : Promise.resolve(new Map()),
      isGse
...
- No padding, no explanations beyond the tables and top pick`;
    ...
    await streamGemini({ contents: prompt, maxOutputTokens: 3000, onChunk: chunkCb });
  };

  await cachedStream(`dashboard:${market}`, DASHBOARD_TTL, run, onChunk);
}

export async function streamSignalScan(market, onChunk) {
  const run = async (chunkCb) => {
    const entries = [];
    for (const [key, mkt] of Object.entries(MARKETS)) {
      if (market && market !== 'ALL' && key !== market) continue;
...
- No padding, no explanations beyond the table and top pick`;
    ...
    await streamGemini({ contents: prompt, maxOutputTokens: 2500, onChunk: chunkCb });
  };

  await cachedStream(`signal:${market}`, SIGNAL_TTL, run, onChunk);
}
...
- Max 350 words total`;
    ...
    await streamGemini({ contents: prompt, maxOutputTokens: 700, onChunk: chunkCb });
  };

  const key = `screener:${market}:${type}:${query.trim().toLowerCase()}`;
  await cachedStream(key, SCREENER_TTL, run, onChunk);
}
